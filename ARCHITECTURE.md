...
## v6.0.0 Growth Engine
- `MBFGrowth` がRelationship、訪問日数、Memory、会話数から成長スコアを計算する。
- Homeは成長段階を単一状態として参照し、芽・若芽・若木・木・大樹を描画する。
- 時間帯と季節はHomeの状態クラスへ反映し、FriendStateとは独立して背景を変化させる。
